...
public class Grid : MonoBehaviour
{
    private void OnGUI() //=Canvas things need to be attachd to this to render on screen or be seen on screen
    {
        ...
        if (GUI.Button(new Rect(ScreenPlacement(6.5f, 5, 3, 1)), "Options"))
        {

        }

        if (GUI.Button(new Rect(ScreenPlacement(6.5f, 6, 3, 1)), "Exit"))
        {
    ...
    public Rect ScreenPlacement(float startPositionX, float startPositionY, float sizeX, float sizeY)
    {
